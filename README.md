...
<details>
<summary>More screenshots — a digest post, the Executive's Guide, Key Dates</summary>

|  |  |
|---|---|
| ![Technical digest post, with the review-and-send tray open](docs/screenshots/digest-post.png) | ![Executive's Guide](docs/screenshots/exec-guide.png) |
| ![Key Dates calendar](docs/screenshots/deadlines.png) | |
...
A companion **Executive's Guide** is generated alongside each digest - plain-language briefings for leadership, compliance officers, and IT directors. A **LinkedIn newsletter draft** (long-form, for the Newsletter edition) and a separate **short announcement/teaser post** are also produced, ready to review and post.
...
flowchart LR
    ...
    E --> L["📨 LinkedIn Newsletter Draft"]
    E --> N["📣 LinkedIn Announcement Draft"]
    F --> H["git push"]
    G --> H
    C --> H
    H --> I["⚡ GitHub Actions\nbuild check only"]
...
| `digest.py` | Reads `pending_draft.json`, calls Claude API (×4) for technical digest, Executive's Guide, LinkedIn newsletter draft, and LinkedIn announcement post; updates health baseline |
...
| `generate_search_index.py` | Builds `site/static/search.json` from all published posts - runs automatically at the end of every `digest.py` publish |
| `generate_graph.py` | Regenerates the Tag Universe globe's data (`site/static/universe/index.html`) from published posts - runs from `weekly-run.sh` right after `digest.py` |
| `dateutils.py` | Shared date parsing for `scraper.py` and `digest.py` - normalizes the inconsistent date formats across feed types |
...
| `combine_posts.py`, `retroactive_exec.py`, `retroactive_reformat.py`, `regen_linkedin_only.py` | One-off maintenance utilities (merging posts, backfilling missed Executive's Guides, reformatting to a new style, regenerating a single LinkedIn draft) - not part of the automated weekly run |
...
| `static/universe/index.html` | Tag Universe - interactive globe of tags/posts, regenerated by `generate_graph.py` each week |
| `static/search.json` | Generated search index, regenerated by `generate_search_index.py` each week |
