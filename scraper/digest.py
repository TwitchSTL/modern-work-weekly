...
- Top 5 prioritization: when multiple items are eligible for Top 5, feature launches, licensing changes, and capability GAs in Intune, Defender, Purview, Entra ID, and Microsoft 365 Business Premium/E5 licensing take priority over a CVE-led story by default. Only let a CVE anchor Top 5 as the lead story when it is actively exploited in the wild (cve_exploited: "Yes") or clearly affects a broad, common environment (e.g. all Windows versions, all Exchange Online tenants) rather than a narrow platform slice (e.g. Mac-only, a single app) with no confirmed exploitation. A Critical-but-unexploited, platform-narrow CVE can still make Top 5 if it clears the eligibility rule above, but should not crowd out stronger licensing/feature news from those five product areas when both are competing for a slot.
...
Pillar page link: OPTIONAL, added 2026-08-29, forward-only, never retrofit to historical posts. If "Why This Week Matters" or another section genuinely uses the phrase "Modern Work" as a concept leadership might want defined (at most once per post), link that single occurrence to `/what-is-modern-work/`. Skip this most weeks; it's an option, not a requirement.
...
- Place a standalone divider line — "⸻" on its own line — between major sections: after the hook line, and between each of ON THE RADAR / WORTH YOUR ATTENTION / HELP DESK / closing line
