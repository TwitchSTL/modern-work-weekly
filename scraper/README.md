# Scraper

Collects Microsoft "What's New" updates and known issues across all tracked portals,
deduplicates against previously seen items, and builds a rolling pending draft for
the Claude API digest pipeline.

---

## Components

| Script | Role |
|---|---|
| `scraper.py` | Fetches all portals, deduplicates, classifies each item into a Modern Work practice area, appends new items to `pending_draft.json`, logs classification stats |
| `digest.py` | Reads `pending_draft.json` (+ `docs_updates.py` output), calls Claude API (×3 — digest, Executive's Guide, LinkedIn draft), writes Hugo posts, regenerates search index, archives draft |
| `sources.py` | All source URLs, RSS feeds, CSS selectors, health-check flags, and `CLASSIFICATION_KEYWORDS` for the Modern Work practice-area taxonomy |
| `docs_updates.py` | Fetches recent commits to Microsoft's documentation GitHub repos, feeding the digest's optional "Documentation Updates" section — catches doc-only changes (retirements, clarified prerequisites) that never appear as a "what's new" announcement |
| `classification_report.py` | Reads `state/classification_stats.json` and prints per-category totals + fallback rate — quick health check on whether the classification taxonomy is catching items correctly |
| `deploy.sh` | 5-minute cron entrypoint — pull → Hugo build → rsync to web root (skips if nothing new) |
| `weekly-run.sh` | Tuesday cron entrypoint — pull → scrape → draft → push → build & deploy |
| `health-run.sh` | 8-hour cron entrypoint — health sources + deadline purge → push if changed |

---

## Setup

```bash
# Venv lives outside the repo, alongside the working scraper directory
mkdir -p /opt/modern-work-weekly/scraper
cd /opt/modern-work-weekly/scraper
python3 -m venv .venv
source .venv/bin/activate
pip install -r /opt/modern-work-weekly/repo/scraper/requirements.txt
```

Set your Anthropic API key on the LXC (never in the repo):

```bash
echo 'ANTHROPIC_API_KEY=sk-ant-...' > /opt/modern-work-weekly/.env
chmod 600 /opt/modern-work-weekly/.env
```

---

## Running manually

```bash
# Normal run — accumulates new items into pending_draft.json
python scraper.py

# Ignore dedup state — pull everything available
python scraper.py --force-all

# Single source only (useful for testing a specific portal)
python scraper.py --source Intune
python scraper.py --source Entra

# Health sources only — updates health.json without touching the digest pipeline
python scraper.py --health-only
```

```bash
# Generate technical digest, Executive's Guide, and LinkedIn draft from current pending draft
python digest.py

# Skip Executive's Guide generation
python digest.py --skip-exec

# Skip LinkedIn newsletter draft generation
python digest.py --skip-linkedin

# Dry run — print the prompt without making an API call
python digest.py --dry-run

# Use a specific draft file instead of pending_draft.json
python digest.py --draft ../state/weekly_draft_2026-05-20.json

# Skip archiving pending_draft.json after publish (useful for testing)
python digest.py --keep-pending
```

---

## Output

| File | Description |
|---|---|
| `../state/pending_draft.json` | Rolling accumulator — all new items since last publish |
| `../state/weekly_draft_YYYY-MM-DD.json` | Per-run snapshot (retained for reference) |
| `../state/archive/pending_draft_YYYY-MM-DD.json` | Draft archived after each digest publish |
| `../site/content/posts/YYYY-MM-DD.md` | Technical digest written by `digest.py` |
| `../site/content/exec/YYYY-MM-DD.md` | Executive's Guide written by `digest.py` |
| `../state/linkedin_draft_YYYY-MM-DD.txt` | LinkedIn newsletter draft written by `digest.py` (manual-post starting point — see `../linkedin/template.md`) |
| `../site/static/search.json` | Static search index, regenerated by `digest.py` after each publish |
| `../site/data/health.json` | Known issues written by `scraper.py` (health sources) |
| `../state/classification_stats.json` | Per-run category counts + fallback rate, written by `scraper.py`'s `write_classification_stats()`; reviewed via `classification_report.py` |

---

## Automated pipeline

### Tuesday 5:55 AM CST — `weekly-run.sh`

1. `git pull origin main` — sync repo to latest
2. `scraper.py --force-all` — fetch all portals, append new items to `pending_draft.json`
3. `digest.py` — call Claude API (×3 — digest, Executive's Guide, LinkedIn draft),
   write draft Hugo posts, regenerate search index, update health baseline, archive pending draft
4. `git commit` + `git push origin main`
5. Build and deploy immediately (`hugo --minify` + rsync) — doesn't wait on the
   `deploy.sh` cron

The draft posts are written with `draft: false` and pushed automatically. Review via
the live site or by pulling the repo after the cron fires.

### Every 5 minutes — `deploy.sh`

1. `git pull origin main` — exits quietly if nothing new
2. If new commits landed: `hugo --minify` then rsync `site/public/` → web root

This is what publishes ordinary pushes to `main` (e.g. editorial corrections).
GitHub Actions only runs a CI build check — it does not deploy.

### Every 8 hours — `health-run.sh`

1. `git pull` — sync repo to latest
2. `scraper.py --health-only` — fetch known-issues sources, write `health.json`,
   purge expired entries from `deadlines.json`
3. `git push` — only if `health.json` content changed (no empty commits)

---

## Dedup logic

Each item gets a stable SHA-256 hash of `source_name::title`. Seen hashes are stored
in `../state/seen_items.json`. On each run, only items with new hashes are appended
to the pending draft. If a page updates the wording of an existing item significantly,
it will appear as a new item — that's intentional.

---

## Adding new sources

Edit `sources.py` — add a new entry to the `SOURCES` list with the portal URL.
Prefer RSS feeds over HTML scraping where available. Set `health=True` for known
issues pages that feed the sidebar health widget.

Sources don't carry a category field — `classify_item()` in `scraper.py` always
classifies each item from its title/body text against `CLASSIFICATION_KEYWORDS`
in `sources.py`, regardless of which source it came from. A new source doesn't
need a category assignment; if its content isn't landing in the expected practice
area, add keywords to `CLASSIFICATION_KEYWORDS` instead and check
`classification_report.py` to confirm the fallback rate drops.
